...
-- UNIQUE(ZoneID, SeatCode) dam bao ma ghe duy nhat trong Zone (BR05).
-- ============================================================
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- SeatCode chi la dinh danh trong mot khu. UPDLOCK + HOLDLOCK khoa ca
        -- key range tren UQ_Seat_Zone_SeatCode den khi INSERT xong, nen hai yeu
        -- cau dong thoi cung tao mot ma trong mot khu khong the cung vuot qua
        -- kiem tra roi de lo ra loi UNIQUE SQL Server tho.
        IF EXISTS (SELECT 1
                   FROM Seat WITH (UPDLOCK, HOLDLOCK)
                   WHERE ZoneID = @TargetZoneID
                     AND SeatCode = @SeatCode)
            THROW 58124, 'sp_CreateSeat: SeatCode da ton tai trong Zone nay.', 1;
        ...
        DECLARE @ZoneStatus VARCHAR(32);
        SELECT @ZoneStatus = ZoneStatus
        FROM Zone WITH (UPDLOCK, HOLDLOCK)
        WHERE ZoneID = @TargetZoneID;

        IF @ZoneStatus <> 'Active'
            THROW 59829, 'sp_CreateSeat: Khong the tao ghe trong Zone da Retired.', 1;
        ...
        IF NULLIF(LTRIM(RTRIM(@SeatRowLabel)), '') IS NULL
        ...
        IF @SeatRowLabel IS NOT NULL
           AND EXISTS (SELECT 1 FROM Seat WITH (UPDLOCK, HOLDLOCK)
                       WHERE ZoneID = @ZoneID
