...
EXEC test.sp_RunTest @Suite,'fn_GetCustomerTicketCount_NoBooking','SUCCESS',NULL,@SQL;
...
EXEC test.sp_RunTest @Suite,'fn_GetCustomerTicketCount_With2Pending','SUCCESS',NULL,@SQL;
...
EXEC test.sp_RunTest @Suite,'fn_CalculateBookingSubtotal_OnlyActive','SUCCESS',NULL,@SQL;
...
SET @bid = SCOPE_IDENTITY();
    ...
    INSERT INTO BookingPromotionApplication (BookingID,PromotionID,DiscountAmount,AppliedTimestamp,ApplicationOrder)
    VALUES (@bid,@pid,200000,SYSDATETIME(),1);
...
EXEC test.sp_RunTest @Suite,'fn_CalculateFinalAmount_Fixed200k','SUCCESS',NULL,@SQL;
...
SET @SQL = N'
    ...
    DECLARE @esid INT = (SELECT TOP 1 EventSeatID FROM EventSeat WHERE InventoryStatus=''Available'' ORDER BY EventSeatID);
    DECLARE @bid INT;
    INSERT INTO Booking (CustomerUserID,ConcertID,BookingStatus,SubtotalAmount,FinalAmount,HoldStartDatetime,HoldExpiryDatetime) VALUES (@uid,@cid,''Pending'',100000,100000,SYSDATETIME(),DATEADD(minute, 15, SYSDATETIME()));
SET @bid = SCOPE_IDENTITY();
    UPDATE EventSeat SET InventoryStatus=''OnHold'' WHERE EventSeatID=@esid;
    INSERT INTO BookingEventSeatAllocation (BookingID,EventSeatID,AllocationStatus,PriceSnapshot)
    VALUES (@bid,@esid,''Active'',100000);
    INSERT INTO BookingPromotionApplication (BookingID,PromotionID,DiscountAmount,AppliedTimestamp,ApplicationOrder)
    VALUES (@bid,@pid,500000,SYSDATETIME(),1);  -- Discount > Subtotal
    DECLARE @final DECIMAL(18,0) = (SELECT FinalAmount FROM dbo.fn_CalculateFinalAmount(@bid));
    IF @final <> 0 BEGIN DECLARE @m4 NVARCHAR(200)=''Expected 0, got ''+CAST(@final AS VARCHAR); THROW 50000, @m4, 1; END;';
EXEC test.sp_RunTest @Suite,'fn_CalculateFinalAmount_ZeroFloor','SUCCESS',NULL,@SQL;
