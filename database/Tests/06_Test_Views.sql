...
EXEC test.sp_RunTest @Suite,'VW_ConcertSalesSummary_Runs','SUCCESS',NULL,@SQL;

SET @SQL = N'DECLARE @n INT = (SELECT COUNT(*) FROM VW_ActiveInventoryStatus);';
EXEC test.sp_RunTest @Suite,'VW_ActiveInventoryStatus_Runs','SUCCESS',NULL,@SQL;

SET @SQL = N'DECLARE @n INT = (SELECT COUNT(*) FROM VW_CustomerBookingHistory);';
EXEC test.sp_RunTest @Suite,'VW_CustomerBookingHistory_Runs','SUCCESS',NULL,@SQL;

SET @SQL = N'DECLARE @n INT = (SELECT COUNT(*) FROM VW_CheckInReport);';
EXEC test.sp_RunTest @Suite,'VW_CheckInReport_Runs','SUCCESS',NULL,@SQL;

SET @SQL = N'DECLARE @n INT = (SELECT COUNT(*) FROM VW_WaitlistQueue);';
EXEC test.sp_RunTest @Suite,'VW_WaitlistQueue_Runs','SUCCESS',NULL,@SQL;

SET @SQL = N'DECLARE @n INT = (SELECT COUNT(*) FROM VW_AuditTrail);';
EXEC test.sp_RunTest @Suite,'VW_AuditTrail_Runs','SUCCESS',NULL,@SQL;
