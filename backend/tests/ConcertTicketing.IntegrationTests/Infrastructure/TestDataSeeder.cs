...
public sealed class TestDataSeeder
{
    ...
    public async Task<int> CreateUserAsync(string role, string passwordHash = "x", int instance = 0)
    {
        ...
        var userId = await _fixture.InsertAdminAsync(@"
    ...
    public async Task<int> CreateArtistAsync()
    {
        ...
        return await _fixture.InsertAdminAsync(
        ...
        string? concertName = null)
    {
        ...
        return await _fixture.QueryAdminAsync<int>(@"
            INSERT INTO Concert (OrganizerUserID, VenueID, ConcertName,
            ...
            -- Trả về @cid đã bắt được NGAY sau khi chèn Concert, không gọi lại
            -- SCOPE_IDENTITY() ở cuối: ConcertArtist không có cột IDENTITY nên giá trị
            -- đó tình cờ vẫn đúng, mà dựa vào sự tình cờ ấy thì thêm một cột IDENTITY
            -- vào ConcertArtist sau này là lặng lẽ trả về sai khoá.
            SELECT @cid;",
